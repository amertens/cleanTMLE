...
#   cleanTMLE:::authorize_outcome_analysis() -> hash-and-fingerprint-bound token,
#   cleanTMLE:::run_clean_tmle_preoutcome()  -> pre-outcome dossier (Y not authorised),
#   cleanTMLE:::run_clean_tmle_primary()     -> refuses to read Y without a valid token.
#
# Guard tests build the `pre` fixture with fit_ps_glm and no plasmode so they
# stay fast; the orchestration test exercises cleanTMLE:::run_clean_tmle_preoutcome().
...
.mk_pre <- function(stop_gate = FALSE, seed = 7L, n = 400L) {
  ...
  audit <- cleanTMLE:::record_checkpoint(
    audit, cleanTMLE:::checkpoint_balance(cleanTMLE:::compute_ps_diagnostics(ps),
                              lock_hash = lock$lock_hash))
  req <- c("Check Point 1", "Check Point 2", "Check Point 3")
  if (!stop_gate) {
    lk2 <- cleanTMLE:::define_negative_control(lock, "nc_outcome")
    nc  <- cleanTMLE:::run_negative_control(lk2, "nc_outcome", ps)
    audit <- cleanTMLE:::record_checkpoint(
      audit, cleanTMLE:::checkpoint_residual_bias(nc, lock_hash = lock$lock_hash))
  }
  gate <- cleanTMLE:::authorize_outcome_analysis(audit, required_stages = req,
                                     allow_flag = TRUE)
...
test_that("the audit fingerprint changes when the audit is modified (tamper-evident)", {
  ...
  a2  <- cleanTMLE:::record_checkpoint(pre$audit, cleanTMLE:::checkpoint_cohort_adequacy(pre$lock))
  g2  <- cleanTMLE:::authorize_outcome_analysis(a2, required_stages = pre$required_stages,
                                    allow_flag = TRUE)
...
test_that("run_clean_tmle_primary refuses when no authorization is available", {
  pre <- .mk_pre()
  pre$gate <- NULL
  expect_error(cleanTMLE:::run_clean_tmle_primary(pre, authorization = NULL),
               "authoriz", ignore.case = TRUE)
})

test_that("run_clean_tmle_primary refuses a STOP gate", {
  pre <- .mk_pre(stop_gate = TRUE)
  expect_error(cleanTMLE:::run_clean_tmle_primary(pre, verbose = FALSE),
...
test_that("run_clean_tmle_primary refuses an authorization minted for a different lock", {
  ...
  expect_error(cleanTMLE:::run_clean_tmle_primary(pre, authorization = bad, verbose = FALSE),
...
test_that("run_clean_tmle_primary refuses when the audit changed after authorization", {
  ...
  pre$audit <- cleanTMLE:::record_checkpoint(pre$audit, cleanTMLE:::checkpoint_cohort_adequacy(pre$lock))
  expect_error(cleanTMLE:::run_clean_tmle_primary(pre, verbose = FALSE),
...
test_that("run_clean_tmle_primary runs the primary TMLE under a valid GO token", {
  pre <- .mk_pre()
  fit <- cleanTMLE:::run_clean_tmle_primary(pre, verbose = FALSE)
...
test_that("run_clean_tmle_primary honors the allow_outcome_access escape hatch", {
  ...
  fit <- suppressWarnings(
    cleanTMLE:::run_clean_tmle_primary(pre, allow_outcome_access = TRUE, verbose = FALSE))
  expect_true(is.numeric(fit$risk_difference))
})
...
test_that("a token minted from an empty audit (NA fingerprint) is refused (fail-closed)", {
  ...
  forged <- cleanTMLE:::authorize_outcome_analysis(cleanTMLE:::create_audit_log(pre$lock),
  ...
  expect_error(
    cleanTMLE:::run_clean_tmle_primary(pre, authorization = forged, verbose = FALSE),
...
test_that("run_clean_tmle_preoutcome builds a dossier without authorizing the outcome", {
  ...
  pre <- cleanTMLE:::run_clean_tmle_preoutcome(
...
test_that("preoutcome -> authorize -> primary is a working two-pass flow", {
  ...
  auth <- cleanTMLE:::authorize_outcome_analysis(pre$audit,
                                     required_stages = pre$required_stages,
                                     allow_flag = TRUE)
  fit <- cleanTMLE:::run_clean_tmle_primary(pre, authorization = auth, verbose = FALSE)
  expect_true(is.numeric(fit$risk_difference))
})
