...
#   cleanTMLE:::decision_thresholds() + dt_* extractors + cleanTMLE:::attach_decision_thresholds()
#   cleanTMLE:::gate_dq() GO/FLAG/STOP
#   cleanTMLE:::authorize_outcome_analysis() masking-bug fix + block_on_any_stop
#   cleanTMLE:::checkpoint_residual_bias(rule = "equivalence")
#   cleanTMLE:::checkpoint_balance(stop_smd) / cleanTMLE:::checkpoint_cohort_adequacy(stop_*)
#   run_plasmode_dq_stress() MAR scenario
#   cleanTMLE:::assess_dgp_fidelity()

# ── decision_thresholds + extractors ─────────────────────────────────────
test_that("decision_thresholds builds a classed object and extractors work", {
  dt <- cleanTMLE:::decision_thresholds(dq_max_abs_bias = 0.03, nco_rule = "equivalence",
                            nco_null_band = 0.025)
  expect_s3_class(dt, "cleantmle_thresholds")
  expect_equal(cleanTMLE:::dt_dq(dt)$max_abs_bias, 0.03)
  expect_equal(cleanTMLE:::dt_nco(dt)$rule, "equivalence")
  expect_equal(cleanTMLE:::dt_nco(dt)$null_band, 0.025)
  expect_true(all(c("max_abs_bias", "coverage_threshold", "se_sd_window") %in%
                    names(cleanTMLE:::dt_plasmode(dt))))
  expect_equal(cleanTMLE:::dt_balance(dt)$stop_smd, 0.20)
  expect_equal(cleanTMLE:::dt_cohort(dt)$stop_n_per_arm, 20L)
})
...
test_that("attach_decision_thresholds fingerprints the rule and preserves lock_hash", {
  ...
  lock2 <- cleanTMLE:::attach_decision_thresholds(lock, cleanTMLE:::decision_thresholds())
  expect_false(is.null(lock2$thresholds_hash))
  expect_identical(lock2$lock_hash, base_hash)         # base hash unchanged
  expect_no_error(cleanTMLE:::validate_analysis_lock(lock2))
  # Different thresholds => different fingerprint.
  lock3 <- cleanTMLE:::attach_decision_thresholds(lock,
                                      cleanTMLE:::decision_thresholds(dq_max_abs_bias = 0.05))
...
test_that("gate_dq returns STOP when a degraded row breaches the bias envelope", {
  cp <- cleanTMLE:::gate_dq(make_dq(bias_deg = 0.03), candidate = "c1",
...
test_that("gate_dq returns GO when all degraded rows are within the envelope", {
  cp <- cleanTMLE:::gate_dq(make_dq(bias_deg = 0.005, rmse_deg = 0.011, cov_deg = 0.93),
...
test_that("gate_dq reads thresholds from a decision_thresholds object", {
  dt <- cleanTMLE:::decision_thresholds(dq_max_abs_bias = 0.02, dq_min_coverage = 0.85,
                            dq_max_rmse_ratio = 1.5)
  cp <- cleanTMLE:::gate_dq(make_dq(bias_deg = 0.03), candidate = "c1", thresholds = dt)
  expect_equal(cp$decision, "STOP")
})
...
test_that("a balance STOP is not masked by a later same-prefix DQ checkpoint", {
  ...
  cp1 <- cleanTMLE:::new_checkpoint("Check Point 1: Cohort Adequacy", "GO",
                        data.frame(x = 1), list(), "ok")
  cp2 <- cleanTMLE:::new_checkpoint("Check Point 2: Treatment Comparability", "STOP",
                        data.frame(x = 1), list(), "bad balance")
  cpdq <- cleanTMLE:::new_checkpoint("Check Point 2c: DQ Stress", "GO",
                         data.frame(x = 1), list(), "dq ok")
  cp3 <- cleanTMLE:::new_checkpoint("Check Point 3: Residual Bias", "GO",
                        data.frame(x = 1), list(), "nc ok")
  audit <- cleanTMLE:::record_checkpoint(audit, cp1)
  audit <- cleanTMLE:::record_checkpoint(audit, cp2)
  audit <- cleanTMLE:::record_checkpoint(audit, cpdq)   # later, shares "Check Point 2" prefix
  audit <- cleanTMLE:::record_checkpoint(audit, cp3)
  g <- cleanTMLE:::authorize_outcome_analysis(audit)
...
test_that("a recorded DQ STOP blocks authorization via block_on_any_stop", {
  ...
  lock <- create_analysis_lock(dat, "treatment", "event_24",
                               c("age", "sex"), seed = 3)
  audit <- cleanTMLE:::create_audit_log(lock)
  audit <- cleanTMLE:::record_checkpoint(audit, cleanTMLE:::new_checkpoint(
  ...
  g_block <- cleanTMLE:::authorize_outcome_analysis(audit, block_on_any_stop = TRUE)
  expect_equal(g_block$decision, "STOP")
  # With the safety net off, the non-required CP2c STOP no longer blocks.
  g_noblock <- cleanTMLE:::authorize_outcome_analysis(audit, block_on_any_stop = FALSE)
...
test_that("equivalence NCO rule passes a precise near-null and flags an imprecise one", {
  cp_ok  <- cleanTMLE:::checkpoint_residual_bias(nc_obj(0.001, 0.005),
                                     rule = "equivalence", null_band = 0.02)
  expect_equal(cp_ok$decision, "GO")
  cp_bad <- cleanTMLE:::checkpoint_residual_bias(nc_obj(0.001, 0.020),
                                     rule = "equivalence", null_band = 0.02)
  expect_equal(cp_bad$decision, "STOP")          # wide CI cannot prove near-null
  expect_error(cleanTMLE:::checkpoint_residual_bias(nc_obj(0, 0.01), rule = "equivalence"))
})
...
test_that("checkpoint_balance STOP floor is configurable via stop_smd", {
  ...
  expect_equal(cleanTMLE:::checkpoint_balance(ps_diag, max_smd = 0.10,
...
test_that("checkpoint_cohort_adequacy STOP floor is configurable", {
  ...
  lock <- create_analysis_lock(df, "treatment", "event_24",
                               c("age", "sex"), seed = 1)
  cp <- cleanTMLE:::checkpoint_cohort_adequacy(lock, min_n_per_arm = 50,
                                   stop_n_per_arm = 20)
...
test_that("assess_dgp_fidelity flags a shifted synthetic covariate", {
  ...
  f_ok <- cleanTMLE:::assess_dgp_fidelity(real, synth_same, c("age", "sex"), "treatment")
  f_bad <- cleanTMLE:::assess_dgp_fidelity(real, synth_shift, c("age", "sex"), "treatment")
